...
def X_stab_Check_surface_code(HX, anc_qubits, anc_coords,all_two_qubit_depol_rates):
    ...
    for k in anc_qubits:
        ...
        temp_probs = all_two_qubit_depol_rates[cnt]
        cnt_G      = 0
        for Q in qubits_to_check:

            X_stab_Check.append("CX",[k,Q])
            X_stab_Check.append("DEPOLARIZE2",[k,Q],temp_probs[cnt_G])
...
def sample_from_lognorm(p: float, std: float):
...
def sample_from_lognorm_two_qubit_depol(p,std,num_CNOT_gates):
...
def planar_surface_code_circuit_X_memory(L: int, num_rounds: int, p_depol_data: float, p_depol_anc:float, std: float, p_depol_gate:float):
    '''Construct the X-memory unrotated surface code circuit, but with error rates that vary across qubits or gates.
    The error rates are sampled from the log-normal distribution.
    ...
    Input:
        ...
        p_depol_after: mean depolarizing rate after 2-qubit gates with variation determined by std (float)
    ...
    n_data            = np.shape(HX)[1]  #Number of data qubits  
    ...
    #Number of CNOT gates is 4 or 3 per check   
    cnt=0
    all_two_qubit_depol_rates = []
    for k in X_anc_qubits:

        qubits_to_check = np.nonzero(HX[cnt,:])[1]
        qubits_to_check = [x+1 for x in qubits_to_check]
        num_CNOT_gates  = len(qubits_to_check)
        temp_probs      = sample_from_lognorm_two_qubit_depol(p_depol_gate,std,num_CNOT_gates)

        all_two_qubit_depol_rates.append(temp_probs)
        cnt+=1

    
    
    X_stab_Check        = X_stab_Check_surface_code(HX, X_anc_qubits, anc_coords_X,all_two_qubit_depol_rates)
