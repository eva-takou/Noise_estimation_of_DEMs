...
def construct_logical_state_on_ancilla_w_Flag(distance: int, logical_state: str,after_CNOT_depol_type: str, p_depol_after:float):
    '''Construct the |0>_L or |+>_L on the ancilla: it should hold that the ancilla qubits start from |0>.
    This functionality is used for the Steane-type syndrome extraction.'''
...
def construct_repeating_block_for_Steane_Extraction_w_Flag(distance: int, data_qubits: list,anc_qubits: list, flag_qubit: list ,p_data: float, 
                                                    p_anc: float, logical_state: str, Reset: bool, after_CNOT_depol_type: str,
                                                    p_depol_after: float, num_rounds: int):
    ...
    if Reset==True:
        ...
        if num_rounds==0:
            ...
            #To measure the GHZ stabilizer of the flag
            block.append("CX",[anc_qubits[-1]]+flag_qubit)
            block.append("DEPOLARIZE2",flag_qubit+[anc_qubits[-1]],p_depol_after)
            block.append("CX",[anc_qubits[1]]+flag_qubit)
            block.append("DEPOLARIZE2",flag_qubit+[anc_qubits[1]],p_depol_after)
            ...
            if distance>3:    
                ...
                block.append_operation("MR",flag_qubit)    
                block.append("DETECTOR",[stim.target_rec(-1)],(0,0,0)) 
            ...
            for k in range(len(anc_qubits)-1,0,-1): #range(len(anc_qubits)-1)
                ...
                # else:
                rec3 = stim.target_rec(-k-(distance)-1)
                rec4 = stim.target_rec(-1-k-(distance)-1)
...
def repetition_code_circuit_Steane_Extraction_W_Flag(distance: int, num_rounds: int, p_data: float, p_anc: float, Reset: bool,
                                              p_depol_after: float, after_CNOT_depol_type: str, logical_state: str):
    '''
    Circuit for Steane-style syndrome extraction on repetition code.
    ***Does not contain extra flag qubit which is needed for d \geq 5 if we want to preserve the distance.***
    ...
    # Prepare the ancilla state (includes a flag qubit too)
    anc_prep_circuit = construct_logical_state_on_ancilla_w_Flag(distance, logical_state,after_CNOT_depol_type, p_depol_after)
    ...
    #Now, construct the repeating block:
    circuit += construct_repeating_block_for_Steane_Extraction_w_Flag(distance, data_qubits,anc_qubits, flag_qubit ,p_data, 
                                                               p_anc, logical_state, Reset, after_CNOT_depol_type,
                                                               p_depol_after, num_rounds)
    ...
    for k in range(len(data_qubits),1,-1): #range(1,len(data_qubits))
        ...
        rec3 = stim.target_rec(-k-distance)
        rec4 = stim.target_rec(-k-distance+1)
